...
public class Pool {
  ...
  public int attack() //Returns the number of hits
  {
    ...
    for (Bead x : beadPool) {
      if (x.getEffect() == 1 && x
          .inHand(turn)) //As a reminder 1 is a black bead and 2 is a blue beed
      {
        blackBeads += 1;
  ...
  public boolean takeHit(
      int hits) //Checks to see things that are in hand first then take damage assuming moving from spent to exhausted, then from exhausted to wounded, and finally from pool to exhausted
  {
    ...
    for (int x = 0; x < hits; x++) {
      ...
      for (int j = 0; j < beadPool.length; j++) //creating the array of priorities
      {
        ...
        } else if (beadPool[j].getState() == BeadState.exhausted) {
          prio[j][0] = 1;
          prio[j][1] = (int) (Math.random() * 10000);
        } else if (beadPool[j].getState() == BeadState.spent) {
          prio[j][0] = 2;
          prio[j][1] = (int) (Math.random() * 10000);
        } else if (beadPool[j].getState() == BeadState.in_pool) {
          prio[j][0] = 3;
          prio[j][1] = (int) (Math.random() * 10000);
        } else {
          prio[j][0] = 4;
      ...
      if (currentHit.getState()
          == BeadState.in_pool) //if it's in your hand it needs to get hit "twice"
      {
  ...
  public boolean checkLiving() //checks to see a bead has a state that is either 0 or 1
  {
    boolean state = false;
    for (Bead x : beadPool) {
      if (x.getState() == BeadState.in_pool || x.getState() == BeadState.spent) {
        state = true;
      }
    }
    return state;
